...
int main(void) {
...
#ifndef _WIN32
    /* A server with metrics can still be the old, non-optimized GLM build.
     * Check the binary contract without loading a model or opening a port. */
    char runtime_dir[] = "/tmp/dstudio-runtime-test.XXXXXX";
    assert(mkdtemp(runtime_dir));
    char previous_ds4[DSTUDIO_PATH_MAX];
    cstr_copy(previous_ds4, sizeof previous_ds4, g_ds4_dir);
    cstr_copy(g_ds4_dir, sizeof g_ds4_dir, runtime_dir);
    char runtime_file[PATH_MAX];
    snprintf(runtime_file, sizeof runtime_file, "%s/ds4.c", runtime_dir);
    FILE *runtime_fp = fopen(runtime_file, "wb");
    assert(runtime_fp);
    fputs("static bool ds4_model_is_glm53", runtime_fp);
    fclose(runtime_fp);
    snprintf(runtime_file, sizeof runtime_file, "%s/ds4-server", runtime_dir);
    runtime_fp = fopen(runtime_file, "wb");
    assert(runtime_fp);
    fputs("decode_tokens_per_second\n decode_elapsed_seconds", runtime_fp);
    fclose(runtime_fp);
    assert(chmod(runtime_file, 0700) == 0);
#ifdef __APPLE__
    assert(!setup_server_metrics_binary_ready());
#else
    assert(setup_server_metrics_binary_ready());
#endif
    runtime_fp = fopen(runtime_file, "ab");
    assert(runtime_fp);
    ...
    cstr_copy(g_ds4_dir, sizeof g_ds4_dir, previous_ds4);
    ...
    assert(!lan_public_path_allowed("POST", "/api/pdf/evidence"));
    assert(!lan_public_path_allowed("OPTIONS", "/api/pdf/evidence"));
