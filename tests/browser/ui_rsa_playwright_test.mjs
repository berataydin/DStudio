...
try {
  ...
  console.log('ui_rsa_playwright_test: playwright missing, NOT RUN');
  process.exit(1);
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/api/agent/send' && req.method === 'POST') {
    ...
    const output = `\u001e${JSON.stringify({ type: 'gsa_phase', phase: payload.phase, payloadJson: raw })}`;
...
try {
  ...
  try {
    ...
    server.close();
    console.log('ui_rsa_playwright_test: browser missing, NOT RUN');
    process.exit(1);
  }
}
