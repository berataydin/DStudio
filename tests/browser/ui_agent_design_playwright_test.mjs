...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/api/agent/send' && req.method === 'POST') {
    ...
    if (/Cowork document-table fixture/.test(body.displayPrompt || '')) {
      const table = {
        format: 'dstudio.document-table.v1', title: 'Course comparison', revision: 1,
        columns: [{ id: 'topic', label: 'Topic', type: 'text' }], totalRows: 1, checkedAt: '2026-09-05T00:00:00Z',
        rows: [{ id: 'course', label: 'Course A', cells: { topic: { status: 'sourced', value: 'Algebra',
          candidates: [{ value: 'Algebra', status: 'sourced', issues: [], evidence: [{ path: 'course.pdf', segment: 'page:2',
            quote: 'Topic: Algebra', sha256: 'a'.repeat(64), match: true }] }] } } }], checks: [], nextOffset: null,
      };
      agentPollText += [
        '\x01USER\x02Cowork document-table fixture\x01ENDUSER\x02\n',
        '\x1e' + JSON.stringify({ type: 'tool_call', name: 'document_table', input: { action: 'inspect', path: 'courses.table.json' } }) + '\n',
        '\x1e' + JSON.stringify({ type: 'tool_result', name: 'document_table', output: JSON.stringify(table) }) + '\n',
        'The course comparison is ready for review.\n',
      ].join('');
      agentPollWorking = false;
    }
...
try {
  ...
  try {
    ...
    server.close();
    console.log('ui_agent_design_playwright_test: browser missing, NOT RUN');
    process.exit(1);
  }
}
...
try {
  ...
  await page.locator('#composer-input').fill('Cowork document-table fixture');
  await page.locator('#btn-send').click();
  const documentTable = page.locator('.document-table-preview').last();
  await documentTable.waitFor({ timeout: 5000 });
  assert.match(await documentTable.innerText(), /Course comparison/);
  await documentTable.locator('.document-table-cell summary').click();
  assert.equal(await documentTable.locator('.document-table-cell summary').evaluate(node => getComputedStyle(node).display), 'block',
    'nested evidence disclosures must not inherit the timeline 21px glyph grid');
  assert.match(await documentTable.innerText(), /course.pdf · page:2/);
  assert.match(await documentTable.innerText(), /Topic: Algebra/);
  await delay(700);
  assert.equal(await documentTable.locator('.document-table-cell[open]').count(), 1, 'idle transcript refresh preserves expanded evidence');
  fs.mkdirSync('tests/.artifacts/document-table', { recursive: true });
  await page.screenshot({ path: 'tests/.artifacts/document-table/cowork-full-app.png', fullPage: true });
